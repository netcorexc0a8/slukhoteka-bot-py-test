...
class BackendAPIClient:
    """
    Клиент к backend API.

    Использует один долгоживущий httpx.AsyncClient с пулом TCP-соединений.
    Инициализируется один раз в bot.py и передаётся через dp["api_client"].

    Для корректного завершения вызовите await client.aclose() при остановке бота.
    """
    ...
    async def aclose(self):
        """Закрывает пул соединений. Вызывается при остановке бота."""
    ...
    async def auth_login(self, phone: str, platform: str, external_id: str, name: str | None = None) -> Dict[str, Any]:
        ...
        if name:
            payload["name"] = name
        response = await self._client.post("/api/v1/auth/login", json=payload)
    ...
    async def services_list(self) -> List[Dict[str, Any]]:
        response = await self._client.get("/api/v1/services")
        response.raise_for_status()
        return response.json()
    ...
    async def clients_get_all(self, user_id: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        response = await self._client.get("/api/v1/clients", params=params)
        response.raise_for_status()
        return response.json()
    ...
    async def client_create(
        ...
        if phone:
            payload["phone"] = phone
        response = await self._client.post("/api/v1/clients", json=payload)
        response.raise_for_status()
        return response.json()

    async def client_can_transfer(self, client_id: int) -> Dict[str, Any]:
        response = await self._client.get(f"/api/v1/clients/{client_id}/can-transfer")
        response.raise_for_status()
        return response.json()

    async def client_transfer(self, client_id: int, new_owner_id: int) -> Dict[str, Any]:
        response = await self._client.post(
            f"/api/v1/clients/{client_id}/transfer",
    ...
    async def subscription_create(
        ...
        payload: Dict[str, Any] = {"client_id": client_id, "service_id": service_id}
        ...
        if notes:
            payload["notes"] = notes
        response = await self._client.post("/api/v1/subscriptions", json=payload)
        response.raise_for_status()
        return response.json()

    async def subscription_update(self, subscription_id: int, **kwargs) -> Dict[str, Any]:
        response = await self._client.put(
            f"/api/v1/subscriptions/{subscription_id}", json=kwargs
        )
        response.raise_for_status()
        return response.json()
    ...
    async def groups_list(self, service_id: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        response = await self._client.get("/api/v1/groups", params=params)
        response.raise_for_status()
        return response.json()

    async def group_get(self, group_id: str) -> Dict[str, Any]:
        response = await self._client.get(f"/api/v1/groups/{group_id}")
        response.raise_for_status()
        return response.json()
    ...
    async def group_create(
        ...
        if time is not None:
            payload["time"] = time
        response = await self._client.post("/api/v1/groups", json=payload)
        response.raise_for_status()
        return response.json()

    async def group_update(self, group_id: str, **kwargs) -> Dict[str, Any]:
        response = await self._client.put(f"/api/v1/groups/{group_id}", json=kwargs)
        response.raise_for_status()
        return response.json()

    async def group_delete(self, group_id: str) -> bool:
        response = await self._client.delete(f"/api/v1/groups/{group_id}")
        return response.status_code == 204

    async def group_add_participant(self, group_id: str, client_id: int) -> bool:
        response = await self._client.post(
            f"/api/v1/groups/{group_id}/participants/{client_id}"
        )
    ...
    async def bookings_for_date(
        ...
        response = await self._client.get("/api/v1/bookings", params=params)
        response.raise_for_status()
        return response.json()
    ...
    async def booking_create(
        ...
        if notes:
            payload["notes"] = notes
        response = await self._client.post("/api/v1/bookings", json=payload)
        response.raise_for_status()
        return response.json()
    ...
    async def booking_delete(self, booking_id: int, actor_id: Optional[int] = None) -> bool:
        ...
        response = await self._client.delete(
            f"/api/v1/bookings/{booking_id}", params=params
        )
        return response.status_code == 204

    async def booking_update(self, booking_id: int, **kwargs) -> Dict[str, Any]:
        response = await self._client.put(f"/api/v1/bookings/{booking_id}", json=kwargs)
        response.raise_for_status()
        return response.json()
    ...
    async def booking_create_recurring(
        ...
        if notes:
            payload["notes"] = notes
        response = await self._client.post("/api/v1/bookings/recurring", json=payload)
        response.raise_for_status()
        return response.json()
    ...
    async def booking_group_move(
        ...
        response = await self._client.post("/api/v1/bookings/group/move", json=payload)
        response.raise_for_status()
        return response.json()
    ...
    # USERS
    # ------------------------------------------------------------------
    async def users_get_all(self, skip: int = 0, limit: int = 100) -> List[Dict[str, Any]]:
        response = await self._client.get(
            "/api/v1/users", params={"skip": skip, "limit": limit}
        )
        response.raise_for_status()
        return response.json()
    ...
    async def users_update(self, user_id: int, name: Optional[str] = None, role: Optional[str] = None) -> Dict[str, Any]:
        ...
        if role is not None:
            payload["role"] = role
        response = await self._client.put(f"/api/v1/users/{user_id}", json=payload)
        response.raise_for_status()
        return response.json()

    async def users_delete(self, user_id: int) -> Dict[str, Any]:
        response = await self._client.delete(f"/api/v1/users/{user_id}")
        response.raise_for_status()
        return {}
    ...
    async def invite_check(self, code: str) -> Dict[str, Any] | None:
        try:
            response = await self._client.get(
                "/api/v1/users/invite/check", params={"code": code}
            )
            if response.status_code == 200:
                return response.json()
            return None
    ...
    async def export_excel(
        ...
        # Увеличиваем таймаут только для этого запроса
        response = await self._client.get(
            "/api/v1/export/excel", params=params, timeout=60.0
        )
        response.raise_for_status()
    ...
    async def sync_to_yandex(self, file_data: bytes, filename: str) -> Dict[str, Any]:
        ...
        response = await self._client.post(
            "/api/v1/sync/yandex", files=files, timeout=60.0
        )
        response.raise_for_status()
        return response.json()
    ...
    async def backup_database(self) -> bytes:
        response = await self._client.get(
            "/api/v1/backup/database", timeout=120.0
        )
        response.raise_for_status()
        return response.content